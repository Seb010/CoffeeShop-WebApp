...
public class OrdersController implements Serializable {
    ...
    private OrdersJpaController jpaController = null;
    ...
    private OrdersJpaController getJpaController() {
        if (jpaController == null) {
            jpaController = new OrdersJpaController(utx, emf);
        }
        ...
        if (pagination == null) {
            pagination = new PaginationHelper(10) {
                ...
                public int getItemsCount() {
                    return getJpaController().getOrdersCount();
                }

                @Override
                public DataModel createPageDataModel() {
                    return new ListDataModel(getJpaController().findOrdersEntities(getPageSize(), getPageFirstItem()));
                }
            };
        }
        return pagination;
    ...
    private void updateCurrentItem() {
        int count = getJpaController().getOrdersCount();
        ...
        if (selectedItemIndex >= 0) {
            current = getJpaController().findOrdersEntities(1, selectedItemIndex).get(0);
        }
    }
    ...
    public SelectItem[] getItemsAvailableSelectMany() {
        return JsfUtil.getSelectItems(getJpaController().findOrdersEntities(), false);
    }

    public SelectItem[] getItemsAvailableSelectOne() {
        return JsfUtil.getSelectItems(getJpaController().findOrdersEntities(), true);
    }
    ...
    public static class OrdersControllerConverter implements Converter {
        ...
        public Object getAsObject(FacesContext facesContext, UIComponent component, String value) {
            ...
            return controller.getJpaController().findOrders(getKey(value));
        }
